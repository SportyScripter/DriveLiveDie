...
def test_login_success():
    ...
    login_data = {"email": "testadmin@example.com", "password": "test123"} # login data for admin user
...
def test_delete_test_adimn(): # test delete admin user
    ...
    assert db_session.query(User).filter(User.email == "testadmin@example.com").first() is None
...
def test_logout_success(): # test logout with token
    ...
    response = client.post("/auth/logout", headers={"Authorization": f"Bearer {access_token}"}) # logout admin user
...
def test_create_user_success():
    ...
    response = client.post("/auth/login", json=login_data) # login admin user
    access_token = response.json()["access_token"] # get access token
    response_create_user = client.post("/auth/create-user", json = test_user, headers = {"Authorization": f"Bearer {access_token}"}) # create user
    logout_response = client.post("/auth/logout", headers={"Authorization": f"Bearer {access_token}"}) # logout admin user
    delete_test_user(db_session , email="testadmin@example.com") # delete admin user
    assert response_create_user.status_code == 200
    assert response_create_user.json() == {"message": "User created successfully", "data": {"username": "testuser", "email": "testuser@example.com"}}
...
def test_delete_test_user(): # test delete user
    ...
    assert db_session.query(User).filter(User.email == "testuser@example.com").first() is None
